...
tf.logging.set_verbosity(tf.logging.ERROR)  #print error/warning on screen
#pointing to the trained model
classifier = tf.estimator.Estimator(model_dir="C:\\Users\Hp\\Documents\\TYminiPROJ\\Sign-Language-master\\tmp\\cnn_model3", model_fn=cnn_model_fn)
prediction = None
model = load_model('C:\\Users\\Hp\\Documents\\TYminiPROJ\\Sign-Language-master\\cnn_model_keras2.h5')

def get_image_size():
	img = cv2.imread('C:\\Users\\Hp\\Documents\\TYminiPROJ\\Sign-Language-master\\gestures\\0\\100.jpg', 0)
	return img.shape
...
def recognize():
	...
	while True:
		...
		image, contours, hierarchy = cv2.findContours(thresh.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)#[0]
		if len(contours) > 0:
